...
for (t in test.order){
  for (sub in subjects.order){
    df.t <- subset(df, test_name == t & grade %in% plain.grades.nok2 & achievement.level != 'U' & subject == sub)
    d.cr <- df.t %>% group_by(school, grade, subject, test_name) %>% do(b_and_above(.))
  	p <- single_test_bar_percs_plot(df.t, d.cr, paste(t, sub, sep=" "))
    save_plot_as_pdf(p, paste0("2014-15 ", t, short_labeller('subject', sub), " 3-8 Scores by Grades"))
