teacher.summary.plot.38 <- function(teacher.name, se.data, gsse.data, star.data,
  ...
  se.t <- subset(se.data, teacher_name == teacher.name)
	se.t.c <- select(se.t, id, teacher_name, achievement_level, subject, test_name, grade, percent, scaled_score)
  ...
  d.props <- se.t.c %>% group_by(grade, subject) %>% do(make_al_table(.)) %>% data.frame()
  ...
  class.counts <- select(se.t, student_id, subject, grade) %>% unique() %>%
    group_by(subject, grade) %>% summarize(n=n())
  class.counts$test_name <- rep('L15', nrow(class.counts))
    ...
    geom_text(data=class.counts, aes(x=test_name, y=1, label=n), size=3.5)+
  ...
  # Add a goal line if there are BOY LEAP scores
  b.above.l14 <- subset(b.above, test=="L14")
  ...
  # Change the color scale if the grades are < 3
  if(max(unique(class.counts$grade)) < 3) {
    p.bars <- p.bars + scale_fill_manual(values=alPalette.light.lows.k2, guide=F)
  }
  ...
  d.obs <- subset(obs.data, name==teacher.name)
	...
	p <- arrangeGrob(p.bars, right.side, ncol=2,
										# widths=c(0.67, 0.33),
										main=paste0("\n2014-15 Teacher Profile - ", teacher.name)
	)
	return(p)
}
...
plot.teacher.summary <- function(teacher.name, se.data, gsse.data, star.data,
  ...
  tryCatch({
    p <- teacher.summary.plot.38(teacher.name, se.data, gsse.data, star.data,
  																	highlights, star.means, obs.data, obs.means,
  																	all.obs.means)
  	pdf(paste0("../output/teacher profiles/", teacher.name, " Profile 2014-15.pdf"), width=10.5, height=7)
  	print(p)
  	dev.off()
  },
    error = function(e){
